...
--radiolist --multiple --column ' ' --column '磁盘')
    ...
    # min efiStart is 2
    echo "efiStart: ${efiStart}"
    if [[ $efiStart -lt 2 ]]; then
        echo "efiStart is less than 2, setting to 2"
        efiStart=2
    fi
    ...
    efi_info="EFI分区: ${efiStart}MiB - ${efiEnd}MiB"
    if [ $single_partition ]; then
        root_info="Root分区: ${rootStart}MiB - 100%"
    else
        root_info="Root分区: ${rootStart}MiB - ${rootEnd}MiB"
        swap_info="Swap分区: ${swapStart}MiB - ${swapEnd}MiB"
        if [ $homeEnd ]; then
            home_info="Home分区: ${swapEnd}MiB - ${homeEnd}MiB"
        else
            home_info="Home分区: ${swapEnd}MiB - 100%"
        fi
    fi

    if zenity --question --text "各分区范围, 确定安装?\n\n${efi_info}\n${root_info}\n${swap_info}\n${home_info}\n" --width=500
    then
        echo "\n开始进行分区..."
    else
        echo "\n没有写入任何内容, \n您取消了非破坏性安装, 请重试"
    ...
    if [[ "${install}" == "3" ]]; then
        echo "删除分区 /dev/${OVERWRITE_DEVICE}"
        parted ${DEVICE} rm ${OVERWRITE_DEVICE_SER}
    fi

    echo "创建EFI分区: ${efiStart}MiB - ${efiEnd}MiB"
